  ...
  req.on('end', () => {
    try {
      ...
      if (jsonData.hasOwnProperty('data')) {
        ...
        //console.log(reformateo)

        // Extraer fport y payload del mensaje
        const { fport, payload } = reformateo;

        // Llamar a la función Decoder
        const result = Decoder(payload.match(/.{1,2}/g).map(byte => parseInt(byte, 16)), fport);

        console.log(result);
        
        // Enviar jsonData por MQTT al tópico 'sw/send'
...
const puerto = 3006;
const direccionIP = '172.16.22.229';
